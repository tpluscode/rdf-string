...
export function namedNode(term: Pick<NamedNode, 'value'>, { base = '', prefixes = {} }: NamedNodeOptions): PartialString {
...
const shorthandSyntaxPatterns = new TermMap<NamedNode, RegExp>([
  [xsd.integer, /^-?[0-9]+$/],
  [xsd.decimal, /^-?[0-9]+\.[0-9]+$/],
...
export function literal(term: Literal, { base = '', prefixes = {} }: NamedNodeOptions): PartialString {
  const ntriplesString = ntriples.literal(term)

  // replace datatype with prefixed name
  const [value, datatype] = ntriplesString.split('^^')
  if (datatype && !shorthandSyntaxPatterns.get(term.datatype)?.test(term.value)) {
    const datatypeResult = namedNode({ value: datatype.slice(1, -1) }, { base, prefixes })

    return {
      value: `${value}^^${datatypeResult.value}`,
